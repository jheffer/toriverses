...
void Harvester::execute() {
  for(unsigned int i(0); i < program.length() && alive; i++) { step(i); } // perform ith command
}
...
void Harvester::show() const {
  ...
  cout << "(" << look(-3,0) << look(-2,0) << look(-1,0) << // down periscope!
  look(0,0) << look(+1,0) << look(+2,0) << look(+3,0) << ") ";
  cout << "E = " << E;
  cout << "\tx = (" << x << ", " << y << ")";
  cout << "\tp = (" << px << ", " << py << ")";
  cout << "\tScore = " << score;
...
string Harvester::getStatus() const { return status; }
